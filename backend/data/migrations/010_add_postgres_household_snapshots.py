"""
Migration 010 (PostgreSQL/Timescale): add household snapshot tables.

Logical purpose:
1. create `household_snapshots`
2. create `tracked_household_history`
3. add indexes for analytical reads
4. convert both tables to hypertables when Timescale is available
...
def run_migration():
    """Execute the Postgres household-snapshot migration."""
    try:
        import psycopg
    ...
    print("=" * 70)
    print("EcoSim Data Warehouse Migration 010 (Postgres Household Snapshots)")
    print("=" * 70)
    print()
    print(f"DSN: {dsn}")
    ...
    try:
        ...
        print("  - household_snapshots hypertable when Timescale is available")
        print("  - tracked_household_history hypertable when Timescale is available")
        print()
        print("=" * 70)
