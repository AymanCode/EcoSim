...
**Warmup unemployment:** During the config-driven warmup phase (`CONFIG.time.warmup_ticks`, currently 10 by default), baseline government firms still do most of the employment bootstrapping. If unemployment rises during this period, the bank correctly does not treat it as a lending problem because government baseline firms do not rely on private credit. This is primarily a baseline firm sizing and warmup calibration issue, not a banking problem.
