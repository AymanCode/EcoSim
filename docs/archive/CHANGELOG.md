...
3. **Leakage-safe, multiplicity-corrected, honest about nulls.** Held-out seeds *and* held-out lever vectors; features strictly ≤ t; persistence/trend baselines that must be beaten with a bootstrap CI clear of zero; Wilcoxon + Holm across policies × outcomes. Distress at t+8 is reported as an honest null (not forecastable better than persistence); profit-tax shows no claimable household effect, with its distress delta measurable but under the effect-size bar. Reporting the nulls is what makes the positive results believable.
