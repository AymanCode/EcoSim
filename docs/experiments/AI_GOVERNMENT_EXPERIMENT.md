...
Raw per-run artifacts are generated by `backend/tools/llm/run_llm_government_test.py` and intentionally left out of the public repository. The curated table above keeps the portfolio-facing result readable without publishing every generated JSON and Markdown output file.
